...
import com.centit.shopping.biz.utils.DBService;
 ...
 */
@Component
public class CheckOrderTask_new {
    private static final Logger LOGGER = LoggerFactory.getLogger(CheckOrderTask_new.class);
        ...
        try{
            ...
            for(ShoppingOrderform shoppingOrderform:orderList){
                ...
                if(shoppingOrderform.getOrderId().startsWith(Const.TICKET_ORDER)){
                    ...
                    //商城订单超时时间改为20分钟
                    if(StringUtil.differentMinutesByMillisecond(addTime)>20){
                        //自动取消订单
...
//                                shoppingGoods = shoppingGoodsDao.queryDetail(shoppingGoods);
                                String propertys = shoppingGoodscart.getPropertys();
                                //数据库增加库存
                                dbUtil.addDBGoodsInventory(shoppingGoodscart.getGoodsId(),propertys,shoppingGoodscart.getCount());
                                //redis增加商品库存
                                if (StringUtil.isNotNull(propertys)) {
                                    String inventory_key = REDIS_KEY_GOODS + shoppingGoodscart.getGoodsId()+":"+propertys;
                                    String goods_key = REDIS_KEY_GOODS + shoppingGoodscart.getGoodsId();
                                    redisStockService.addStockWithPropertys(goods_key,inventory_key,shoppingGoodscart.getCount());
                                }else{
                                    String goods_key = REDIS_KEY_GOODS + shoppingGoodscart.getGoodsId();
                                    redisStockService.addStock(goods_key,shoppingGoodscart.getCount());
                                }
...
//                                shoppingArtactivity = shoppingArtactivityDao.queryDetail(shoppingArtactivity);
                                String propertys = shoppingGoodscart.getPropertys();
                                dbUtil.addDBActInventory(shoppingGoodscart.getGoodsId(),propertys,shoppingGoodscart.getCount());
                                //redis增加商品库存
                                if (StringUtil.isNotNull(propertys)) {
                                    String inventory_key = REDIS_KEY_ACT + shoppingGoodscart.getGoodsId()+":"+propertys;
                                    String goods_key = REDIS_KEY_ACT + shoppingGoodscart.getGoodsId();
                                    redisStockService.addStockWithPropertys(goods_key,inventory_key,shoppingGoodscart.getCount());
                                }else{
                                    String goods_key = REDIS_KEY_ACT + shoppingGoodscart.getGoodsId();
                                    redisStockService.addStock(goods_key,shoppingGoodscart.getCount());
...
//                                shoppingArtplanDao.updatePlanNum(shoppingArtplan);
                                String propertys = shoppingGoodscart.getPropertys();
                                dbUtil.addDBPlanInventory(shoppingGoodscart.getGoodsId(),propertys,shoppingGoodscart.getCount());
                                //redis增加商品库存
                                if (StringUtil.isNotNull(propertys)) {
                                    String inventory_key = REDIS_KEY_PLAN + shoppingGoodscart.getGoodsId()+":"+propertys;
                                    String goods_key = REDIS_KEY_PLAN + shoppingGoodscart.getGoodsId();
                                    redisStockService.addStockWithPropertys(goods_key,inventory_key,shoppingGoodscart.getCount());
                                }else{
                                    String goods_key = REDIS_KEY_PLAN + shoppingGoodscart.getGoodsId();
                                    redisStockService.addStock(goods_key,shoppingGoodscart.getCount());
